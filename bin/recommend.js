...
import fetchTradableCoins from '../lib/fetch-tradable-coins.js';

import { Base as strategy } from '../strategies/index.js';

const { logTypes, baseCurrency, intervals, minimumTicksPerInterval, timeTillRecommendationExpires } = defaults({
...
async function getCoinData(symbol, index, totalSymbols) {
	return new Promise(async (resolve) => {
		...
		await Promise.all(intervals.map((interval) => {
			...
			// filter coins that have even a single interval type with less than 100 ticks
