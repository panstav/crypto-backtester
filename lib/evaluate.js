...
export default function evaluate({ name: strategyName, advices }, { coinSymbol, ticks, initialCapital, numTicksToEvaluate, percentageToRisk, percentageToGrab }) {
	...
	function simulate_history_onto_strategy() {
		...
		ticks
			...
			.reduce((ticksHistory, tick) => {
				...
				if (advices.buy.every((term) => term(ticksHistory))) trade(buy, tick, percentageToRisk);
				if (advices.sell.every((term) => term(ticksHistory))) trade(sell, tick, percentageToGrab);
	...
	function sell(coinsToSell, sellPrice, humanDate) {
		...
		const coinsLeft = positions
			...
			.reduce((coinsLeftToSell, { buyPrice, buyTime, coinsBought, tradedBack }, index) => {
				if (coinsLeftToSell === 0) return coinsLeftToSell;
				...
				// mark trade
				trades.push({ coinsTraded: untradedCoinsInPosition, buyPrice, sellPrice, buyTime, sellTime: humanDate });
	...
	function summerizeTrades(trades) {
		return trades.map(({ coinsTraded, buyPrice, sellPrice, buyTime, sellTime }) => {
