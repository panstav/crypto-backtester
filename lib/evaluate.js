...
export default function evaluate({ name: strategyName, advices }, { coinSymbol, ticks, initialCapital, numTicksToEvaluate, percentageToRisk, percentageToGrab }) {
	...
	function buy(numberOfCoins, buyPrice, { humanDate, openTime, closeTime }) {
		...
		if (humanDate.includes('2/2021')) {
			debugger;
		}

	}

	function sell(coinsToSell, sellPrice, { openTime, closeTime, humanDate }) {
	...
	function summerizeTrades(trades) {
		const withHours = config.interval.includes('h');
		return trades.map(({ coinsTraded, holdDuration, buyPrice, sellPrice, buyTime, sellTime, change$ }) => {
			...
			return {
				coinsTraded,
				holdDuration: holdDuration / 1000 / 60 / 60,
