...
export default function evaluate({ strategy, coinSymbol, ticks, interval }) {
	const {
		numTicksToEvaluate: proposedNumTicks,
		initialCapital,
		percentageToRisk,
		percentageToGrab,
		timestampToTradeAfter,
		stepSize
	} = defaults();
	const numTicksToEvaluate = proposedNumTicks || ticks.length;
	...
	function simulate_history_onto_strategy() {
		...
		ticks
			...
			.reduce((ticksHistory, tick) => {
				...
				// avoid trading if
				if (
					// current tick is before earliestDate
					tick.closeTime < timestampToTradeAfter
					// all history is to be evaluated and we're at the first few
					|| (numTicksToEvaluate === ticks.length && ticksHistory.length < (stepSize * 2))
				) return ticksHistory;

				if (strategy.advices.buy.every((term) => term(ticksHistory, positions))) trade(buy, tick, percentageToRisk);

				const hasOpenPositions = positions.some(({ resolved }) => !resolved);
				if (hasOpenPositions && strategy.advices.sell.every((term) => term(ticksHistory, positions))) trade(sell, tick, percentageToGrab);
	...
	function log_strategyXcoin_results() {
		log('final-strategy-summary', `\n${coinSymbol} => ${strategy.name}`, { style: 'bold' });
	...
	function summerizeTrades(trades) {
		...
		return trades.map(({ coinsTraded, holdDuration, buyPrice, sellPrice, buyTime, sellTime, change$ }) => {
			...
			return {
				coinsTraded,
				daysHeld: new Decimal(holdDuration).div(1000).div(60).div(60).div(24).toDecimalPlaces(1).toNumber(),
