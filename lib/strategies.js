...
function trending(direction, { MA7, MA14, MA28, continiously = 1, factor = 0 }) {
...
function stochKnD({ cross: crossDireciotn, top = 80, bottom = 20, kOverD = 0 }) {
	return (ticks) => {
		if (ticks.length < 3 || ticks.slice(-3).some(({ StochRSI_02_D }) => !StochRSI_02_D && StochRSI_02_D !== 0)) return false;
...
function keeper(percentageStep) {
	return (ticks, positions) => {
		...
		return (new Decimal(currentBid).minus(buyPrice)).div(precentOffBuyPrice).abs().floor()
			.lessThan((new Decimal(yesterdayBid).minus(positions[positions.length - 1].buyPrice)).div(precentOffBuyPrice).abs().floor())
	}
}
...
export default [
	...
	{
		name: 'Stoch02 - Prevalent(3,10,100) K&D Cross SellOn3 - Infinite positions',
		isActive: false,
		advices: {
			buy: [
				stochKnD_bottom_cross,
	...
	{
		name: 'Stoch02 - Prevalent10 K&D Cross SellFor5 - Infinite positions',
		isActive: false,
		advices: {
			buy: [
				stochKnD_bottom_cross,
