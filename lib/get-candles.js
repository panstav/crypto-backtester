...
export default async function getCandleData(coinSymbol) {
	...
	try {
		...
		if (err.code === 'ENOENT') return downloadAllCandleData(fileName, coinSymbol, 0); // 1530403200000
		throw err;
	}
...
export async function downloadAllCandleData(fileName, coinSymbol, startTime, accu = []) {
	...
	// fetch data
	const rawTicks = await download(coinSymbol, startTime);
	...
	const richTicks = enrich(rawTicks.map(tickDataNamer), { stepSize, interval });
