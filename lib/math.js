...
// export function getStochRSI(ticks) {
// 	const highest = Decimal.max(...ticks.map(({ highPrice }) => highPrice)).toNumber();
// 	const lowest = Decimal.min(...ticks.map(({ lowPrice }) => lowPrice)).toNumber();
// 	const StochK = new Decimal(100).times(new Decimal(ticks[ticks.length - 1].closePrice).minus(lowest)).div(new Decimal(highest).minus(lowest)).toNumber();
// 	// const StochK = new Decimal(new Decimal((new Decimal(ticks[ticks.length - 1].closePrice).minus(lowest)).div(new Decimal(highest).minus(lowest)))).times(100).toNumber();
// 	const last3StochK = ticks.slice(-3, -1).map(({ StochK }) => StochK).concat(StochK);
...
export function getStochRSI(RSIs) {
	const minRSI = Decimal.min(...RSIs);
	return new Decimal(new Decimal(RSIs[RSIs.length - 1]).minus(minRSI)).div(new Decimal(Decimal.max(...RSIs)).minus(minRSI)).toNumber();
}
